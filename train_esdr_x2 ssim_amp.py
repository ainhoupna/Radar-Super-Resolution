...
EPOCHS = 60           
PATIENCE = 7          
ALPHA_SSIM = 0.2      # Weight for SSIM loss
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

CHECKPOINT_ROOT_DIR = "checkpoints_edsr"
MLFLOW_EXPERIMENT_NAME = "AEMET_Radar_EDSR"
# =================================================
...
def main():
    ...
    with mlflow.start_run(run_name=RUN_NAME):
        ...
        train_loader = DataLoader(RadarDataset(TRAIN_DIR, SCALE_FACTOR), batch_size=BATCH_SIZE, shuffle=True, num_workers=4)
        val_loader = DataLoader(RadarDataset(VAL_DIR, SCALE_FACTOR), batch_size=BATCH_SIZE, shuffle=False, num_workers=4)
        ...
        for epoch in range(EPOCHS):
            ...
            for lr, hr in loop:
                ...
                # 3. Scale loss and backward pass
                scaler.scale(loss).backward()
                
                # 4. Unscale and Optimizer step
            ...
            with torch.no_grad():
                for lr, hr in val_loader:
                    lr, hr = lr.to(DEVICE), hr.to(DEVICE)
                    # Use autocast also in validation for consistency and speed
                    with torch.cuda.amp.autocast():
                        sr = torch.clamp(model(lr), 0, 1)
                    val_psnr += calculate_psnr(sr.float(), hr).item() # PSNR usually needs float32
            ...
            print(f"Epoch {epoch+1}: Loss {avg_train_loss:.5f} | PSNR {avg_psnr:.2f} | SSIM {avg_ssim:.4f}")
